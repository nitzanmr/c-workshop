...
//        printf("%d\n", i);
        if (flag_p == 1) {/*checks for the value of the flag of p and adds the text that comes afterwards to the */
            ...
            new_client->text = (char*)malloc((size_of_text+1)*sizeof(char));
            ...
            if((strstr(kkkk[i],"http://")!= NULL)){
                new_client->url = (char*)malloc((strlen(kkkk[i])+2)*sizeof (char));
                // bzero(new_client->url,strlen(new_client->url));
                malloc_url = 1;
...
//                                strncat(new_client->path,&kkkk[i][j],1);/*adds the / to the path at place 0.*/
                                ...
                                int counter = 0;
                                new_client->path = (char*)malloc(strlen(kkkk[i])-j+1);
                                sprintf(new_client->path,"%s",kkkk[i]+j-1);
